...
const EditProfile = ({isOpen, onClose, initialData, onSave, loading}) => {
  ...
  return (
    <div 
      className={`${editProfile.overlay} ${isOpen ? editProfile.show : ""}`} 
      onClick={onClose}
    >
      <div className={editProfile.modal}  onClick={(e) => e.stopPropagation()}>
        <h3 style={{ textAlign: 'center', marginBottom: '30px', fontSize:'21px' }}>Edit profile</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
           <div className={editProfile.inputGroup}>
            ...
            <div className={editProfile.inputGroup}>
                <input type="number" name="phone" value={localData.phone} onChange={handleChange} required />
                <label>Phone</label>
            </div>
          
            <div className={editProfile.inputGroup}>
                <textarea name="address" value={localData.address} onChange={handleChange} rows="3" required />
                <label>Address</label>
            </div>
        </div>
