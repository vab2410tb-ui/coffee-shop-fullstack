...
const Profile = () => {
    ...
    return (
        <div >
                ...
                <li style={{listStyle: 'none', marginLeft: 'auto'}}>
                    <div className={profile.down}>
                        <div style={{display: 'flex'}} >
                            ...
                            <FontAwesomeIcon icon={faChevronDown} className={profile.iconDown} style={{fontSize:'20px'}}/>
                        </div>
            ...
            <div style={{ backgroundColor: '#f6f6f6', marginInline: '400px', borderRadius: '20px', padding: '35px 20px'}}>
                
                    
                {message && <div style={{ padding: '10px', backgroundColor: '#d4edda', color: '#155724', marginBottom: '15px', borderRadius: '4px', textAlign: 'center' }}>{message}</div>}
                {error && <div style={{ padding: '10px', backgroundColor: '#f8d7da', color: '#721c24', marginBottom: '15px', borderRadius: '4px', textAlign: 'center' }}>{error}</div>}

                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        <div>
                            <label style={{ fontSize: '14px', color: 'rgba(0,0,0,0.56)' }}>Name: </label>
                            <FontAwesomeIcon icon={faPen} onClick={() => setIsEditOpen(true)} style={{ cursor: 'pointer', marginLeft: '20px', fontSize: '13px' }}/>
                    ...
                    <EditProfile 
                        ...
                        onSave={(updatedData) => {submitHandler(updatedData)}}
                        loading={loading} />
                </div>
