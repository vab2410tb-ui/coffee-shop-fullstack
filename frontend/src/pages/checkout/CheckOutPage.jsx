...
const CheckoutPage = () => {
  ...
  return (
    <div className={styles.checkout_container}>
      <PageTitle title="Checkout" description="Complete your order at NabCoffeeShop" />
      <div style={{ display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
        <Link to="/">
          <img src="/icon/image.png" alt="NabCoffeeShop" />
        </Link>
        <h1 style={{ fontSize: '16px', borderBottom: '1px solid #3333331f' }}>
          Natural And Blance Joint Stock Company
        </h1>
      </div>
      {userInfo ? (
            ...
            <div className={styles.payment_container}>
              {ACCORDION_PAYMENT.map((method) => (
                <div key={method.id} className={styles.payment_item}>
                  ...
                  <div
                    className={`${styles.payment_content} ${paymentMethod === method.id ? styles.show : ''}`}
                  >
                    <p>{method.content}</p>
                    {method.img && <img src={method.img} alt="QR Code" />}
                  </div>
                </div>
              ))}
            </div>
